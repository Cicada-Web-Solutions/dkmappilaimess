/* =============================================================
   script.js
   DK's Mappillai Mess — Main Site Script
   ---------------------------------------------------------------
   Used by : index.html (homepage), menu.html (full menu page)
   Depends : GSAP 3.12 + ScrollTrigger (loaded via CDN in HTML)
   ---------------------------------------------------------------
   SECTIONS IN THIS FILE:
     1.  menuData          — Full menu data array (18 categories)
     2.  signaturePicks    — Marquee carousel items
     3.  iconLibrary       — SVG icon strings for signature cards
     4.  foodSVGs          — SVG food illustrations (Build Your Virundhu)
     5.  Helpers           — $(), $$(), feature-detection flags
     6.  Navbar / Hamburger — Mobile nav toggle (homepage only)
     7.  Time Cards        — Craving Clock accordion (homepage only)
     8.  Signature Icons   — getSignatureIcon() + renderSignaturePicks()
     9.  Menu Rendering    — renderFilters(), renderMenu(), search debounce
     10. Build Your Virundhu — addFoodToLeaf(), reset button
     11. GSAP Animations   — hero entrance, scroll reveals, hover effects
     12. Back to Top       — Scroll listener + click animation
     13. Contact Us Button — Click animation (navbar red button)
   ============================================================= */

/* =============================================================
   SECTION 1 — MENU DATA
   Full menu for DK's Mappillai Mess — 18 categories.
   Structure: [{ category: String, items: [{ name, type, price, bestSeller? }] }]
   Filter types: "veg" | "non-veg"
   To add a new category: push a new object to this array.
   To add items: push to the matching category's items array.
   ============================================================= */
const menuData = [
  // ── BREAKFAST ─────────────────────────────────────────────
  {
  ...
  // ── DRINKS (Cool / Shakes / Mojito / Juice) ─────────────
  {
...
/* =============================================================
   SECTION 2 — SIGNATURE PICKS
   Format: ["Title", "Tagline", "#CardBgColor", "assets/icon-file"]
   The 4th element is the icon image path — edit per entry to
   ensure the correct image always matches the correct food.
   ============================================================= */
const signaturePicks = [
  ["Mappillai Meals",    "Full leaf energy",      "#CFF5A2", "assets/meals_icon.avif"],
  ["Chicken Biryani",   "Spice pot favourite",   "#FFC7C7", "assets/chicken_biryani_icon.avif"],
  ["Mutton Biryani",    "Weekend royal mood",    "#FFE08A", "assets/mutton_biryani_icon.avif"],
  ["Fish Fry",          "Crisp coastal kick",    "#BBDDFB", "assets/fish_fry_icon.webp"],
  ["Parotta Varieties", "Layered mess magic",    "#FFD47A", "assets/parotta_varieties_icon.webp"],
  ["Grill Chicken",     "Charred and juicy",     "#F7A1CF", "assets/grill_chicken_icon.webp"],
  ["Chettinad Gravy",   "Pepper fire bowl",      "#D8C8FF", "assets/chettinad_gravy_icon.avif"],
  ["Rose Milk",         "Pink chill classic",    "#FBCFE8", "assets/rose_milk_icon.webp"],
  ["Virgin Mojito",     "Minty lime spark",      "#B9F6CA", "assets/virgin-mojito-icon.webp"],
  ["Filter Coffee",     "Steam top finish",      "#DDB892", "assets/filter_coffee_icon.webp"],
];

/* =============================================================
   SECTION 3 — ICON LIBRARY
   SVG string icons used in the signature pick sticker cards.
   Keys match the card category type.
   ============================================================= */
const iconLibrary = {
...
/* =============================================================
   SECTION 4 — SVG FOOD ILLUSTRATIONS
   Used by the "Build Your Virundhu" interactive builder.
   Each key matches a data-food button attribute.
   To add a new food: add a key here AND a button in index.html.
   ============================================================= */
...
/* =============================================================
   SECTION 5 — HELPERS & FEATURE DETECTION
   $()  = querySelector shorthand
   $$() = querySelectorAll shorthand
   hasGsap / hasScrollTrigger = graceful-degrade flags
   ============================================================= */
...
/* =============================================================
   ...
   Toggles mobile nav open/closed. Closes nav on link click.
   ============================================================= */
if (isHomePage) {
  ...
  /* ===========================================================
     SECTION 7 — TIME CARDS / CRAVING CLOCK (Homepage only)
     Accordion: clicking a card expands it and closes others.
     =========================================================== */
...
/* =============================================================
   SECTION 8 — SIGNATURE ICONS & CAROUSEL RENDER
   Icons are embedded directly in each signaturePicks entry (index 3)
   so the mapping is always exact — no % arithmetic drift.
   ============================================================= */
function renderSignaturePicks() {
  const track = $("#signature-track");
  // Double the array to create a seamless infinite-loop marquee
  const looped = [...signaturePicks, ...signaturePicks];
  track.innerHTML = looped
    .map(
      ([title, text, color, icon], index) => `
        <article class="signature-card" tabindex="0" style="--tilt:${index % 2 ? "1.3deg" : "-1.1deg"};--sticker:${color};">
          <div class="sticker-top">
            <span class="sticker-icon" aria-hidden="true">
              <img src="${icon}" alt="${title}" class="sticker-icon-img">
            </span>
...
/* =============================================================
   SECTION 9 — MENU RENDERING (Homepage only)
   renderFilters() — builds category filter buttons (once), then
                     toggles active class on re-call.
   renderMenu()    — filters menuData by activeCategory + search
                     term and injects HTML into #menu-groups.
   Search is RAF-debounced to avoid per-keystroke reflow.
   ============================================================= */
...
/* =============================================================
   SECTION 10 — BUILD YOUR VIRUNDHU (Interactive Builder)
   addFoodToLeaf(food) — injects an SVG food illustration onto
     the banana leaf canvas with random position + GSAP pop-in.
   Reset button — animates all food items out then removes them.
   ============================================================= */
...
/* =============================================================
   SECTION 11 — GSAP ANIMATIONS
   runAnimations() handles:
     - Hero word-by-word entrance animation
     - ScrollTrigger reveal animations for each .reveal section
     - Floating food doodles (hero visual)
     - Signature marquee auto-scroll
...
/* =============================================================
   ...
   - Passive scroll listener toggles .is-visible after 300px.
   - Click: GSAP spring squish + arrow bounce + smooth scroll.
   - CSS ripple class fires on click for visual feedback.
   ============================================================= */
(function () {
...
/* =============================================================
   SECTION 13 — CONTACT US BUTTON (Navbar red CTA)
   - Click: GSAP squish-press + spring-back bounce.
   - .is-clicked class fires a white overlay flash via CSS.
   ============================================================= */
(function () {
